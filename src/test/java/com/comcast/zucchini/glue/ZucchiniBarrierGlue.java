...
import com.comcast.zucchini.test.BarrierTest;
...
public class ZucchiniBarrierGlue {
    ...
    public void second_barrier() throws Throwable {
        LOGGER.debug(String.format("BarrierCount for [%s] is %d, and numContexts is %d after sync", name(), barrierCount,
                BarrierTest.numContexts));
        if (barrierCount != BarrierTest.numContexts) {
            Assert.fail(String.format("The internal barrierCount[%d] does not match the number of contexts[%d]", barrierCount,
                    BarrierTest.numContexts));
        }
        Barrier.sync();
        synchronized (this.getClass()) {
